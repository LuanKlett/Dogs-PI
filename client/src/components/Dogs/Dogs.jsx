...
export default function Dogs({ dogs, order, filterTemperaments, filterDb }) {
  ...
  function filterDogsByTemperament(arr) {
    filterTemperaments.forEach(
      (t) =>
        (arr = arr.filter(
          (d) => d.temperament && d.temperament.includes(t) === true
        ))
    );
    return arr;
  }
  ...
  function handleArrow(e) {
    ...
    if (e.target.value === ">" && pageNumber < pages.length - 1)
  ...
  const [sort, setSort] = useState(
    orderSort(filterDogsByDb(filterDogsByTemperament(dogs)), order)
  );
  ...
  if (pages.length) {
    ...
    return <p style={{ width: "100vw" }}>No se encontraron resultados</p>;
  }
}
